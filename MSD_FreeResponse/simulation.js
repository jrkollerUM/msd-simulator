...
function drawPlot(nowSec) {
  ...
  const pad = { t: 18 * dpr, r: 18 * dpr, b: 72 * dpr, l: 58 * dpr };
...
// ── Keyboard velocity adjustment ───────────────────────────────────────────
animCanvas.addEventListener('keydown', e => {
  if (e.key !== 'ArrowUp' && e.key !== 'ArrowDown') return;
  e.preventDefault();

  const step = e.shiftKey ? 0.1 : 0.5;
  const prefix = animWhich === 2 ? 'r2' : 'r1';
  const current = Math.min(50, Math.max(-50, getNum(`${prefix}-v0`)));
  const next = Math.min(50, Math.max(-50, current + (e.key === 'ArrowUp' ? step : -step)));
  setVal(`${prefix}-v0`, next.toFixed(3));
  update();
});
